...
The argumentative theory is formulated on the basis of a vast amount of experimental evidence, and its core idea is that  the notion of argumentation can serve a foundational role in cognitive science. The theory holds that human reasoning evolved to facilitate efficient argumentation, and that the function of reason is not in arriving at logically correct forms of inference but to contribute to social interaction in such a way as to maximize the positive effect of  deliberation. This, for instance, can serve to shed light on why humans so often reason in a way that most theories would judge to be fallacious. Sometimes they do so because fallacies can be useful in the context of argumentation.1
Notice that this involves a notion of argumentation which is conceptually distinct from that found in traditional argumentation theory, going back to (Toulmin 2003) (first edition from 1958). Here, the approach tends to be more strictly normative, focusing on recognizing and categorising fallacies and on designing argumentation schemes and models that are meant to facilitate sound and rational reasoning, particularly about what arguments we should accept. The argumentative theory, on the other hand, asks us to shift attention towards attempting to understand the nature of what I will call rational consensus, the outcome that is aggregate socially from the various arguments put forth by individual reasoners. This appears to be the crucial normative and foundational aspect of the theory, and we will attempt to shed light on it by describing the space of deliberative outcomes using mathematical tools, and by studying logics that allow us to reason about this space with varying degrees of expressive power.
The formal work will be based on the theory of argumentation frameworks, introduced in (Dung 1995). This formalism relies on representing argumentative structures as directed graphs over arguments where all arguments have edges to all the arguments that they attack. This simple picture facilitates the investigation of a whole range of interesting semantics (Baroni and Giacomin 2007).
...
I conclude that argumentation frameworks are highly suited as a technical starting point towards logics for argumentative deliberation. The crucial challenge that remains, and which will be addressed in this project, is a mechanism for formally introducing an appropriate kind of multi-agent dynamics, suitable for representing the argumentative theory.
The key idea: Introducing a separate layer of subjective representation of argumentative meaning
Much work on multi-agent argumentation has already been carried out in a formal and semi-formal context, but this work is mostly based on a traditional view of argumentation theory. We believe this is implicit in recent formal work such as that of  (M. Caminada, Pigozzi, and Podlaszewski 2011; M. Caminada and Pigozzi 2011) and even more so in the survey of the field given in (Rahwan 2009). In my opinion, however, this view is inappropriate when attempting to formalize the argumentative theory.
The problem is that the representation of the argumentation scenario is fixed and not open to dispute and dynamic change, except with respect to the question of how it should be evaluated. But to model the argumentative theory, we need to depart from this starting point, since it is crucial that the basic representation of the surrounding semantic reality is itself a subjective construction, distinctly produced in each individual agent. To accomplish this, the fundamental idea is to use argumentation frameworks as models of the agents' internal view of the relevant arguments and how they are related. In terms of each individual agent, using terminology from cognitive science, it places the argumentation framework at the informational level of cognitive processing, where previous work have already shown that logical tools can have a particularly crucial role to play, also serving to shed new light on established truths arrived at through empirical work, see e.g., (Stenning and van Lambalgen 2005).
