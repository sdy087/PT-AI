...
Certainly, this is a neat and uncontroversial depiction of the actual utterance made. Moreover, looking at the AF in (3), forgetting about the agent labels, we conclude that $p, r$ must be assigned the value true, so that $\bar r, w$ are both defeated and become false. Hence we conclude that agent ${b}$ won the debate. But is this really the rational way of looking at things?  Applying an argumentation semantics directly to the actual exchange between $a$ and $b$, it would seem so. But intuitively speaking, it seems problematic. Why would a puddle be conclusive evidence, and stronger evidence than a weather report? No, it seems that a clear answer of who won the debate cannot be provided at all; for an objective audience, a weather report might carry some weight, and so might a puddle, but neither seem particularly conclusive.

We believe the problem is with the representation we gave in Figure \ref{fig:wrong}, and the fact that it failed to include information about the agents' view of each other's utterances. In a perfect world, this might not matter, since all debate might eventually be settled conclusively by brute empirical fact, such as observing actual rain as opposed to consulting weather reports and puddles. However, in such a world, deliberation would certainly not be very interesting, and it is not how it tends to play out in the real one. Rather, a debate involves crucially a search for common ground, and common ground depends crucially on how agents perceive the statements made by others, as they reflect on the totality of the debate. This is why we need to be explicit about subjective views, and always ask for a representation of how each individual agent interprets the semantic meaning of all those claims that are relevant to the scenario at hand.
...
Importantly, since we now use AFs to encode subjective views, we are in a position to reject the AF that was used to encode the actual exchange of arguments. This AF encodes an interpretation that takes the attack $(p,w)$ to encode correct semantic information about $p$ and $w$. But according to his view, agent $a$ disagrees with agent $b$ about the presence of this attack, so it is unwarranted to take it for granted that $(p,q)$ will come to influence the outcome of deliberation. Indeed, in order to even begin talking about the outcome we need first some aggregated view on the semantic meaning of the arguments involved. Depending on what this interpretation is taken to look like, the outcome will be different, and while it might be unclear how such an interpretation should be constructed, it seems clear that it needs to be based on the agents' views, not on the actual events that took place during the debate.
